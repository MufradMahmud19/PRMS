...
def create_patient():
    ...
    # Invalidate cache for patient list and individual patient
    invalidate_cache('get_all_patients:():{}', f'get_patient:({patient.patient_id},):{{}}')
...
def login():
    try:
        data = request.get_json()
        print(f"Login attempt with data: {data}")  # Debug output
        if not data or 'username' not in data or 'password' not in data:
            return {'error': 'Missing username or password'}, 400
        
        user = User.query.filter_by(username=data['username']).first()
        print(f"Found user: {user}")  # Debug output
...
def protected():
    try:
        ...
        print(f"Protected route - Current user ID: {current_user_id}")  # Debug output
        user = User.query.get(current_user_id)
        if not user:
            print(f"Protected route - No user found with ID: {current_user_id}")  # Debug output
            return {'error': 'User not found'}, 404
        print(f"Protected route - Found user: {user.to_dict()}")  # Debug output
